...
export class ExtensionsMethod {
    ...
    public static round(value: number, digits: number) {
        if (digits >= 0) return value.round(digits);
    ...
    public static parseExceptionFromServer(pException): any {
        if (pException instanceof Error) return pException.message;
    ...
    public static handleException(pError: Error) {
        if (pError instanceof DOMException) return new Error(pError.message);
    }
    ...
    public static renderImage(
        ...
        if (!String.isNullOrEmpty(imageName)) {
            if (_bIconWeb) {
                ...
                let _zUrlImg: string;
                if (imageName.includes('.')) _zUrlImg = imageName;
                else _zUrlImg = `${imageName}.${extensionName}`;
        ...
        if (width != -1) {
            ...
            if (_bIconWeb) _imgElement.style.fontSize = width + 'px';
        }
    ...
    public static deserializebase64(base64: string, pbAllowCache: boolean = true) {
        ...
        if (xhr.responseText) {
            ...
            if (pbAllowCache) this._cacheRtf.set(_zKey, _zValue);
    ...
    public static rtfRenderer(pzContent: string) {
        ...
        if (this._cacheRtf.has(_key)) return this._cacheRtf.get(_key);
        ...
        if (_m && _m.length > 0)
            for (let _n = 0; _n < _m.length; _n++) {
                ...
                _result +=
                    ...
                    _value +
                    '<div class="rep-content rep-cell-merge merge-' +
                    _indexStyleName +
                    '">\n';
                _result += _rowHtml;
...
String.prototype.toUnicode = function () {
    ...
    for (let i = 0; i < this.length; i++) {
        ...
        while (_partial.length !== 4) _partial = '0' + _partial;
...
String.prototype.indexOfAny = function (anyOf: Array<string>, startIndex?: number, count?: number) {
    ...
    if (count == null) count = _str.length - 1 - startIndex;
...
String.compare = function (
    ...
    if (!wjc.isString(value)) value = String.format('{0}', value);

    if (options == SensitivityEnum.Variant) return value.localeCompare(value1);

    return value.localeCompare(value1, BravoCulture.ci, {
        sensitivity: options
    });
};
    ...
    switch (datePart) {
        ...
        case DatePartEnum.q: {
            return (
                ...
                (_endDate.getMonth() - 1) / 3 -
                (_startDate.getMonth() - 1) / 3
            );
        }
        ...
        case DatePartEnum.m: {
            return (
                12 * _endDate.getFullYear() +
                _endDate.getMonth() -
                (12 * _startDate.getFullYear() + _startDate.getMonth())
            );
        }
...
Date.prototype.toLocaleStringWidthFormat = function (format?: string): string {
    if (!format) return wjc.Globalize.formatDate(this, Date.defaultFormat);
    else return wjc.Globalize.formatDate(this, format);
};
