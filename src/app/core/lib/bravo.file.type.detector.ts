...
export class BravoFileTypeDetector {
    ...
    public static detectFileType(pStream: Uint8Array): FileTypeEnum {
        ...
        if ((_bom[0] == 0xef && _bom[1] == 0xbb && _bom[2] == 0xbf) || // utf-8 
            ...
            (_bom[0] == 0x2b && _bom[1] == 0x2f && _bom[2] == 0x76)) // utf-7
        {
            return FileTypeEnum.Txt;
        }

        for (let _i = 1; _i < this.Signatures.length; _i++) {
            if (String.isNullOrEmpty(this.Signatures[_i]))
                continue;
            ...
            for (const _zSign of _s) {
                ...
                if (_buff == null/*  || _buff.length != _nLength */)
                    continue;

                let _bMatched = true;
                for (let _nByteIndex = 0; _nByteIndex < _buff.length; _nByteIndex++) {
                    if (_keys[_nByteIndex] == "??") // ignore byte
                        continue;
                ...
                if (_type == FileTypeEnum.Zip) {
                    try {
                        ...
                        try {
                            ...
                            else if (_zt.containsEntry("FixedDocumentSequence.fdseq") ||
    ...
    public static getMediaTypeFromFileType(fileType: FileTypeEnum) {
        switch (fileType) {
            case FileTypeEnum.Pdf: return 'application/pdf';
            case FileTypeEnum.Bmp: return 'image/bmp';
            case FileTypeEnum.Jpg: return 'image/jpeg';
            case FileTypeEnum.Gif: return 'image/gif';
            case FileTypeEnum.Tif: return 'image/tiff';
            case FileTypeEnum.Png: return 'image/png';
            case FileTypeEnum.Psd: return '';
            case FileTypeEnum.Wmf: return 'image/wmf';
            case FileTypeEnum.Ico: return 'image/vnd.microsoft.icon';
            case FileTypeEnum.Mp3: return 'audio/mpeg';
            case FileTypeEnum.Avi: return 'video/x-msvideo';
            case FileTypeEnum.Swf: return 'application/x-shockwave-flash';
            case FileTypeEnum.Flv: return 'video/x-matroska';
            case FileTypeEnum.Mp4: return 'application/mp4';
            case FileTypeEnum.Mov: return 'video/x-sgi-movie';
            case FileTypeEnum.Wmv: return 'video/x-ms-wmv';
            case FileTypeEnum.Wma: return 'audio/x-ms-wma';
            case FileTypeEnum.Zip: return 'application/zip';
            case FileTypeEnum.Docx: return 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
            ...
            case FileTypeEnum.Pptx: return 'application/vnd.openxmlformats-officedocument.presentationml.presentation';
            case FileTypeEnum.Xps: return 'application/vnd.ms-xpsdocument';
            case FileTypeEnum.Rar: return 'application/vnd.rar';
            case FileTypeEnum.Doc: return 'application/msword';
            case FileTypeEnum.Rtf: return 'application/rtf';
            case FileTypeEnum.Xls: return 'application/vnd.ms-excel';
            case FileTypeEnum.Ppt: return 'application/vnd.ms-powerpoint';
            case FileTypeEnum.Eps: return 'image/x-eps';
            case FileTypeEnum.Wav: return 'audio/wav';
            case FileTypeEnum.Flac: return 'audio/flac';
            case FileTypeEnum.Mkv: return 'video/mp4';
            case FileTypeEnum.Xml: return 'application/xml';
            case FileTypeEnum.Msi: return 'application/x-msi';
            case FileTypeEnum.Txt: return 'text/plain';
        }
    }
