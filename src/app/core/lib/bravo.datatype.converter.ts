...
export function convertTypeToDbType(type: TypeCode): DbType {
    ...
    else if (type == TypeCode.ByteArray)
...
export class BravoDataTypeConverter {
    public static isDateTimeValue(pValue: any, outArgs: { datetime: Date }, culture?: string) {
        ...
        if (outArgs.datetime instanceof Date)
            return true;
    ...
    public static convertValue(value: any, type: TypeCode, culture?: string) {
        ...
        if (type != TypeCode.String) {
            ...
            if (type == TypeCode.Object && wjc.isString(value))
                return value;
    ...
    public static isMomentValidDate(value: string, pzFormat?: string, strictMode = true) {
        ...
        if (String.isNullOrEmpty(pzFormat))
            _momentDate = moment(value);
        else
            _momentDate = moment(value, this.replaceFormatString(pzFormat), strictMode);

        if (_momentDate && _momentDate.isValid())
            return _momentDate;
        return false;
    }
    ...
    public static compareValue(val1: any, val2: any, type: TypeCode, pbIgnoreCaseString: boolean = false) {
        ...
        if (type == TypeCode.String) {
            ...
            if (compareStrings(val1, val2, pbIgnoreCaseString))
                return true;
        }
        ...
        if (type == TypeCode.ByteArray && val1 && val2) {
            return String.compare(Convert.toBase64String(val1),
                Convert.toBase64String(val2)) == 0;
        }
        ...
        for (const key in enumType) {
            if (key == value || (wjc.isString(key) && wjc.isString(value) && key.toLowerCase() == value.toLowerCase())) {
                if (pOutArgs) {
                    ...
                    if (!isNaN(_key))
                        pOutArgs.resultValue = _key;
                    else
                        pOutArgs.resultValue = enumType[key];
                }
    ...
    public static toSize(pzSize: string) {
        if (String.isNullOrEmpty(pzSize) || !pzSize.includes(','))
            return;
    ...
    public static toPoint(pzPoint: string) {
        if (String.isNullOrEmpty(pzPoint) || !pzPoint.includes(','))
            return;
