...
export function setCurrentEditItem(pCollection: any, pDataRow: WebDataRow, value: any) {
    let _tb: any;
    if (wjc.tryCast(pCollection, 'IWebDataView'))
        _tb = pCollection.table;
    else if (wjc.tryCast(pCollection, 'IWebDataTable'))
        _tb = pCollection;
    ...
    if (_content && !_content.flag && !String.isNullOrEmpty(_content.key) && _cv.currentPosition != -1) {
        ...
        if (_item[_content.key] != _e.ProposedValue) {
            ...
            if (_cv._edtClone)
                _cv._edtClone[_content.key] = _e.ProposedValue;
        }
...
export function buildSort(cv: wjc.CollectionView, pzSort: string) {
    ...
    for (const _orderExpr of _exprs) {
        if (String.isNullOrEmpty(_orderExpr))
            continue;
        ...
        if (_dv)
            ...
            _tb = wjc.tryCast(cv, 'IWebDataTable');

        if (!_tb || !_tb.columns.contains(_zColName))
            continue;
